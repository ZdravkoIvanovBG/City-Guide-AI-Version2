...
export default function Register() {
  ...
  const onSubmit = async (values: z.infer<typeof formSchema>) => {
    try {
      const result = await registerMutation.mutateAsync({ data: values });
      ...
      <div className="flex-1 flex items-center justify-center p-6 mt-20">
        <div className="w-full max-w-md">
          <div className="text-center mb-10">
            ...
            <h1 className="font-serif text-4xl mb-3">Begin Your Journey</h1>
            <p className="text-muted-foreground">Create an account to save your itineraries.</p>
          </div>
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      ...
                      <FormControl>
                        <Input placeholder="Jane Doe" className="bg-background/50 border-border rounded-none focus-visible:ring-primary focus-visible:border-primary" {...field} />
                      </FormControl>
                      <FormMessage />
                    </FormItem>
                  )}
          ...
          <p className="text-center mt-8 text-sm text-muted-foreground">
            Already have an account? <button onClick={() => setLocation("/login")} className="text-primary hover:underline">Sign in.</button>
          </p>
        </div>
      </div>
    </motion.div>
